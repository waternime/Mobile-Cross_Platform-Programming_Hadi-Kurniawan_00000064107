...
type CounterSnapshot = Pick<
  CounterState,
  "totalSuccess" | "totalFailed" | "supabase" | "notification"
>;
...
const buildCounterSnapshot = (
  counters: CounterSnapshot,
  operation: OperationName,
  result: OperationResult
): CounterSnapshot => {
  const snapshot: CounterSnapshot = {
    totalSuccess: counters.totalSuccess + (result === "success" ? 1 : 0),
    totalFailed: counters.totalFailed + (result === "failed" ? 1 : 0),
    supabase: { ...counters.supabase },
    notification: { ...counters.notification },
  };

  snapshot[operation][result] += 1;
  return snapshot;
};

const formatCounterSummary = (counters: CounterSnapshot) =>
  `${counters.totalSuccess} successful, ${counters.totalFailed} unsuccessful.\n` +
  `Supabase: ${counters.supabase.success} success, ${counters.supabase.failed} failed.\n` +
  `Notification: ${counters.notification.success} success, ${counters.notification.failed} failed.`;
...
export default function Index() {
  ...
  const showDatabaseNotification = async (
    ...
    const body = `${formatCounterSummary(counterSnapshot)}\n${formatCoordinates(
      coords
    )}${detail ? `\n${detail}` : ""}`;
  ...
  const sendTrackedNotification = async (
    status: "success" | "failed",
    coords: Coordinates,
    baseCounters: CounterSnapshot,
    detail?: string
  ) => {
    const notificationCounters = buildCounterSnapshot(
      baseCounters,
      "notification",
      "success"
    );
    ...
    try {
      await showDatabaseNotification(status, coords, notificationCounters, detail);
      dispatch(
        recordResult({
  ...
  const uploadToSupabase = async () => {
    ...
    try {
      ...
      const nextCounters = buildCounterSnapshot(counters, "supabase", "success");
      dispatch(
        recordResult({
          operation: "supabase",
          result: "success",
      ...
      const nextCounters = buildCounterSnapshot(counters, "supabase", "failed");
      dispatch(
        recordResult({
          operation: "supabase",
          result: "failed",
      ...
      await sendTrackedNotification("failed", location, nextCounters, message);
      Alert.alert("Error", message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>Week 13 - Redux Notifications</Text>
